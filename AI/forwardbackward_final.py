...
def forward_backward(evidence):
    ...
    for evidence_day in evidence:
        ...
        fw_messages.append(around(prior_fw, decimals=3))
        #Running backwardpart and appending to bw_messages
        prior_bw = backward(prior_bw, evidence_day)
        bw_messages.append(around(prior_bw,decimals=3))
    #Smoothing based on fw_messages and reversed bw_messages
    for i in range(len(fw_messages)):
        print around(normalize(smoothing(fw_messages[i],bw_messages[len(bw_messages)-i-1])),decimals=3)
...
def smoothing(bw_message, fw_message):
...
print 'Part B1:'
print around(forward_testing(evidence1), decimals=3)

print 'Part B2'
print around(forward_testing(evidence2), decimals=3)
